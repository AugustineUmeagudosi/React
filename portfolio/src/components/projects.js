import React, {Component} from 'react';
import {Tabs, Tab, Grid, Cell, Card, CardTitle,CardText, CardActions, CardMenu, Button, IconButton} from 'react-mdl';

class Projects extends Component {
    constructor(props){
        super(props);
        this.state = {activeTab: 0};
    }

    toggleCategories(){
        if(this.state.activeTab === 0){
            return(
                <div className='projects-grid'>
                    {/* project 1 */}
                    <Card shadow={5} style={{minWidth: '450', margin: 'auto'}}>
                        <CardTitle style={{color: '#fff', height: '174px', background: 'url(data:image/png;base64,) center/ cover'}}>This is react</CardTitle>
                        <CardText>
                            This is one of my favorite projects because i get to learn something different annd challenging in a very short period of time!
                        </CardText>
                        <CardActions border>
                            <Button colored>Github</Button>
                            <Button colored>CodePen</Button>
                            <Button colored>Live Demo</Button>
                        </CardActions>
                        <CardMenu style={{color: '#FFF'}}>
                            <IconButton name='share'/>
                        </CardMenu>
                    </Card>

                    {/* project 2 */}
                    <Card shadow={5} style={{minWidth: '450', margin: 'auto'}}>
                        <CardTitle style={{color: '#fff', height: '174px', background: 'url(data:image/png;base64,) center/ cover'}}>This is react project 2</CardTitle>
                        <CardText>
                            This is one of my favorite projects because i get to learn something different annd challenging in a very short period of time!
                        </CardText>
                        <CardActions border>
                            <Button colored>Github</Button>
                            <Button colored>CodePen</Button>
                            <Button colored>Live Demo</Button>
                        </CardActions>
                        <CardMenu style={{color: '#FFF'}}>
                            <IconButton name='share'/>
                        </CardMenu>
                    </Card>

                    {/* project 3 */}
                    <Card shadow={5} style={{minWidth: '450', margin: 'auto'}}>
                        <CardTitle style={{color: '#fff', height: '174px', background: 'url(data:image/png;base64,) center/ cover'}}>This is react project 3</CardTitle>
                        <CardText>
                            This is one of my favorite projects because i get to learn something different annd challenging in a very short period of time!
                        </CardText>
                        <CardActions border>
                            <Button colored>Github</Button>
                            <Button colored>CodePen</Button>
                            <Button colored>Live Demo</Button>
                        </CardActions>
                        <CardMenu style={{color: '#FFF'}}>
                            <IconButton name='share'/>
                        </CardMenu>
                    </Card>
                </div>
            );
        }else if(this.state.activeTab === 1){
            return(
                <div><h1>Hello from the world of JavaScript</h1></div>
            );
        }else if(this.state.activeTab === 2){
            return(
                <div><h1>We Laravel the PHP</h1></div>
            )
        }else if(this.state.activeTab === 3){
            return(
                <div><h1>What a noding JS</h1></div>
            )
        }else if(this.state.activeTab === 4){
            return(
                <div><h1>Ouch! is that a python?</h1></div>
            )
        }else if(this.state.activeTab === 5){
            return(
                <div><h1>Strapping my boots for the camp</h1></div>
            )
        }else{
            return(
                <div><h1>This is literarily MySQL</h1></div>
            )
        }
    }

    render() {
        return(
            <div className = 'category-tabs'>
                <Tabs activeTab={this.state.activeTab} onChange={(tabId) => this.setState({activeTab: tabId})} ripple>
                    <Tab>React</Tab>
                    <Tab>JavaScript</Tab>
                    <Tab>PHP</Tab>
                    <Tab>NodeJS</Tab>
                    <Tab>Python</Tab>
                    <Tab>BootStrap</Tab>
                    <Tab>MySQL</Tab>
                </Tabs>
                
                <section>
                    <Grid className = 'projects-grid'>
                        <Cell col = {12}>
                            <div className='content'>{this.toggleCategories()}</div>
                        </Cell>
                    </Grid>
                </section>
            </div>
        );
    }
}

export default Projects;